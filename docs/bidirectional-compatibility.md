...
| Bloomee | Validated legacy-v2 `.blm` JSON | ArchiveTune only | Existing dedicated route |
...
| ArchiveTune | Native merged `.backup` with a user-supplied ArchiveTune target | Metrolist | Implemented direct target route; preserves the selected target settings and ArchiveTune-only tables |
...
The existing Bloomee route is intentionally narrower: it produces the documented legacy-v2 portable structure after translating ArchiveTune YouTube Music IDs into Bloomee’s resolver namespace. Native Bloomee Isar/MDBX snapshots are not a portable interchange format and remain unavailable in the browser-only runtime.
