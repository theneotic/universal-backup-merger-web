...
function Brand() {
  return <Link href="/" className="flex min-w-0 items-center gap-3" aria-label="Universal Backup Merger home">
    <span className="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-[#14212b] text-[#f5c77a] shadow-[0_12px_25px_rgba(20,33,43,0.15)]"><Archive className="size-5" /></span>
    <span className="min-w-0"><span className="block truncate font-display text-lg font-bold tracking-[-0.04em]">Archive Merge</span><span className="block truncate text-xs font-medium tracking-[0.08em] text-[#7a766d] uppercase">Universal backup merger</span></span>
  </Link>;
}
...
export function SiteHeader() {
  ...
  return <header className="sticky top-0 z-40 border-b border-[#d9d7cf]/80 bg-[#f6f4ef]/92 backdrop-blur-xl">
    <div className="container flex min-h-20 items-center justify-between gap-4">
      ...
      <button type="button" className="inline-flex size-10 items-center justify-center rounded-xl border border-[#d8d3c7] bg-white text-[#14212b] lg:hidden" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} onClick={() => setOpen(value => !value)}>{open ? <X className="size-5" /> : <Menu className="size-5" />}</button>
    </div>
    {open && <div className="border-t border-[#e2ded5] bg-[#f6f4ef] px-4 py-3 lg:hidden"><nav aria-label="Mobile primary navigation" className="container grid gap-1">{mainLinks.map(link => <a href={link.href} onClick={() => setOpen(false)} className="rounded-xl px-4 py-3 text-sm font-semibold text-[#435058] hover:bg-white" key={link.label}>{link.label}</a>)}<a href="/#merge" onClick={() => setOpen(false)} className="mt-2 rounded-xl bg-[#b5672c] px-4 py-3 text-center text-sm font-bold text-white">Start merge</a></nav></div>}
  </header>;
}
...
export function SiteFooter() {
  ...
  return <footer className="border-t border-[#d9d7cf] bg-[#edeae2]">
    ...
    <div className="border-t border-[#d8d4ca]"><div className="container flex flex-col gap-2 py-4 text-xs text-[#747a79] sm:flex-row sm:items-center sm:justify-between"><p>© {year} Universal Backup Merger. All rights reserved.</p><p className="inline-flex items-center gap-1.5"><ChevronDown className="size-3.5 rotate-[-90deg]" /> Backup files stay on-device during processing.</p></div></div>
  </footer>;
}
